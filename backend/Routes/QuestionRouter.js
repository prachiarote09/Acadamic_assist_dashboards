...
const {deleteQuestion, getAllQuestions } = require("../Controllers/QuestionController")

// Set up multer storage for PDF and Image files
const storage = multer.memoryStorage();
const upload = multer({ storage });

// POST route for uploading Exam question
router.post("/", upload.fields([{ name: "pdf", maxCount: 1 }, { name: "img", maxCount: 1 }]), async (req, res) => {
  try {
    ...
    res.status(201).json({ message: "question uploaded successfully!", data: newquestion });
  } catch (err) {
    console.error("Error uploading question:", err);
    res.status(500).json({ message: "Failed to upload question. Please try again later." });
  }
});
