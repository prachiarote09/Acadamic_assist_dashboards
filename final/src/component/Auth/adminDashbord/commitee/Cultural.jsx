...
const Cultural = () => {
  ...
  // Fetch cultural committee members
  const fetchCulturalData = async () => {
    try {
      ...
      handleError("Error fetching cultural committee data");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post("http://localhost:8080/api/cultural/", formData, {
        headers: {
          "Authorization": localStorage.getItem('token'),
          "Content-Type": "application/json"
        }
      ...
      if (response.status === 201) {
        ...
        handleReset();
        fetchCulturalData(); // Refresh the table data
      }
    } catch (error) {
  ...
  const handleDelete = async (id) => {
    try {
      const response = await axios.delete(`http://localhost:8080/api/cultural/${id}`, {
        headers: {
          "Authorization": localStorage.getItem('token'),
          'Content-Type': 'application/json'
        }
            ...
            <tbody>
              {loading ? (
                ...
                culturals.map((cultural) => (
                  <tr key={cultural._id} className="border-b border-gray-200 hover:bg-purple-50">
                    <td className="p-3">{cultural.name}</td>
                    <td className="p-3">{cultural.department}</td>
                    <td className="p-3">{cultural.year}</td>
                    <td className="p-3">{cultural.grNumber}</td>
                    <td className="p-3">
                      <button
                        onClick={() => handleDelete(cultural._id)}
